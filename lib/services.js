...
const credentials = process.env.LOCALSTACK_HOSTNAME ? {
  accessKeyId: "123",
  secretAccessKey: "xyz"
} : undefined;
const endpoint = process.env.LOCALSTACK_HOSTNAME ? `http://${process.env.LOCALSTACK_HOSTNAME}:4592` : undefined;
var sts           = new AWS.STS({
  region: currentRegion,
  endpoint,
  credentials
}); // STS service for impersonation of roles in the fan-out process 
...
function ensureService(target, role, callback) {
  ...
  if(role.services[serviceKey].length == 0) {
    ...
    const endpoint = process.env.LOCALSTACK_HOSTNAME ? `http://${process.env.LOCALSTACK_HOSTNAME}:` : undefined;
    var options = { region: serviceRegion, endpoint, credentials };
    if(role.credentials) {
